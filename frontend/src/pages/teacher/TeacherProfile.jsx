...
export default function TeacherProfile(){
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError("");
      try {
        const [res, classResponse] = await Promise.all([
          authAPI.verifyToken(),
          authAPI.verifyToken().then(userRes => {
            if (userRes.data?.assignedClass) {
              return adminAPI.getClass(userRes.data.assignedClass);
            }
            return null;
          })
        ]);
  ...
  const avatarSrc = useMemo(() => {
    ...
    return "https://ui-avatars.com/api/?name=" + encodeURIComponent(me?.name || "Teacher") + "&background=ffffff&color=000000&size=150";
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-5 flex flex-col items-center text-center">
          <div className="relative">
            ...
            />
            <div className="absolute bottom-1 right-1 w-7 h-7 rounded-full bg-purple-600 text-white flex items-center justify-center text-xs font-bold">
              ✓
            </div>
          </div>
          <h2 className="mt-3 text-xl font-bold text-gray-900">
            {loading ? "Loading..." : form.name || "Teacher"}
          </h2>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl p-3">
                ...
                <div className="text-sm font-semibold text-gray-900 truncate max-w-[60%]">
                  {classData?.department?.name || "Not assigned"}
                </div>
              </div>
              <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl p-3">
